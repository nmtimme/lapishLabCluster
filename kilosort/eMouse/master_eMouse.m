...
% fpath    = 'C:\DATA\Spikes\eMouse\'; % where on disk do you want the simulation? ideally and SSD... % Nick replaced this line with the next one
fpath    = [pwd,filesep]; % where on disk do you want the simulation? ideally and SSD...
if ~exist(fpath, 'dir'); mkdir(fpath); end

% This part adds paths (Nick replaced these lines with the next group)
...
% This part adds paths (Nick replaced the group above with these lines
pathToYourConfigFile = pwd; % for this example it's ok to leave this path inside the repo, but for your own config file you *must* put it somewhere else!  
cd ..
cd ..
addpath(genpath(pwd)) % path to kilosort folder and npy-matlab folders
cd kilosort
cd eMouse
...
mkdir preAutoMerge
rezToPhy(rez, [fpath,'preAutoMerge',filesep]); % Nick changed \ to filesep to be operating system independent
...
% save python results file for Phy
mkdir postAutoMerge
rezToPhy(rez, [fpath,'postAutoMerge',filesep]); % Nick changed \ to filesep to be operating system independent
