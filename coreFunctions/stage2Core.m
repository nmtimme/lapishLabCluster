...
for iShank = 1:(max(probeChanShankID))
    ...
    % Preallocate space for the data
    shankData = NaN([nChannels,nT]);
    geo = NaN([nChannels,2]);
    info = cell([nChannels,1]);
    ...
    for iProbeChan = 1:length(probeChanList)
        ...
        if chanRuling(oeChan) == 0
            ...
            [shankData(iProbeChan,:),~,info{iProbeChan}] = load_open_ephys_data_faster([dcDataSetDir,'/100_CH',num2str(oeChan),'.continuous']);
            
            if ismember(dataSetParams{4},[1,2])
                shankData(iProbeChan,:) = shankData(iProbeChan,:) - chanRef;
            end
            
        elseif chanRuling(oeChan) == 1
            % Quiet the channel
            
            % Fill the shank data for this channel with small non-zero
            % numbers
            shankData(iProbeChan,:) = (10^(-10))*randn(1,nT);
            
        else
            error('No allowed channel ruling provided in stage 1 post review.')
        end
    ...
    % Remove all non-accepted channels
    shankData(tempStage1Ruling ~= 0,:) = [];
    geo(tempStage1Ruling ~= 0,:) = [];
    info(tempStage1Ruling ~= 0) = [];
    tempStage1Ruling(tempStage1Ruling ~= 0,:) = [];
    nChannels = size(shankData,1);
    
    % Add the stage 1 channel rulings
    neurStage1Ruling{iShank} = tempStage1Ruling;
    
    disp(['Finished loading the raw data for shank ',num2str(iShank),'. It took ',num2str(toc,3),' seconds.'])
...
%     end
    ...
    save(fullfile(ops.root,['rezStage1_shank',num2str(iShank),'.mat']), 'rez', '-v7.3');
...
% machines.
save([dcDataSetDir,filesep,'Stage2PreReview',dataSetID],'waveformWindowSize','vBounds','shankGeo','shankWaveforms','neurFR','neurRefVio','neurNSpk','neurAC','neurSTA','neurList','neurDynFR','raster','neurStage1Ruling','shankAveWaveforms','shankDistWaveforms','shankWaveformDifs')
save([mainDC,filesep,'Stage2ResultsPreReview',filesep,'Stage2PreReview',dataSetID],'waveformWindowSize','vBounds','shankGeo','shankWaveforms','neurFR','neurRefVio','neurNSpk','neurAC','neurSTA','neurList','neurDynFR','raster','neurStage1Ruling','shankAveWaveforms','shankDistWaveforms','shankWaveformDifs')
